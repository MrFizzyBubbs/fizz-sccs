...
void doDmt() {
	if ($familiar[Machine Elf].fights_today < $familiar[Machine Elf].fights_limit) {
		...
		while ($familiar[Machine Elf].fights_today < $familiar[Machine Elf].fights_limit) {
			...
			boolean needJoy = !have($item[abstraction: joy]) && !have($effect[Joy]);
			boolean needCertainty = !have($item[abstraction: certainty]) && !have($effect[Certainty]);
			if (!have($item[abstraction: action]) && needJoy) {
				// banish triangle, macro to square, force drops
				assert(!have($effect[Everything Looks Yellow]), "Unable to use yellow ray");
				ensureMp(150);
				adv1($location[The Deep Machine Tunnels], -1, mNew().mDmtSquare());
			} else if (needJoy) {
				// banish square, macro to circle, throw square abstraction, force drops
				check($item[abstraction: action]);
				adv1($location[The Deep Machine Tunnels], -1, mNew().mDmtCircle());
			} else if (needCertainty) {
				// banish square, macro to triangle, throw circle abstraction
				check($item[abstraction: thought]);
				adv1($location[The Deep Machine Tunnels], -1, mNew().mDmtTriangle());
			} else {
